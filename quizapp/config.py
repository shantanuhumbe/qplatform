...
GRADER_SYSTEM_INSTRUCTION = (
    ...
    "CRITICAL RULE FOR IS_CORRECT: If the student's selected option letter matches the correct answer option, `is_correct` MUST be true. "
    "If it does not match, `is_correct` MUST be false. Correctness of the selection is independent of whether reasoning was provided.\n"
    "If the student selected the correct option but did not provide reasoning (or if user_explanation is empty or 'No reasoning provided.'), "
    "set `is_correct` to true, set `conceptual_score` to 10, and in the feedback text, acknowledge their correct choice and gently suggest "
    "that providing reasoning helps identify conceptual alignment.\n"
